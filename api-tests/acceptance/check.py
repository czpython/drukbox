"""Acceptance from inside the box, one provider per run. README.md says what to set."""
...
def get_setting(name: str) -> str:
    return os.environ.get(name) or sys.exit(f"set {name}")


SERVICE_URL = get_setting("SERVICE_URL").rstrip("/")
SERVICE_TOKEN = get_setting("SERVICE_TOKEN")
PROVIDER = get_setting("PROVIDER")
ISSUER_URL = get_setting("ISSUER_URL").rstrip("/")
ISSUER_BEARER = get_setting("ISSUER_BEARER")
ISSUER_CONTROL_URL = os.environ.get("ISSUER_CONTROL_URL", ISSUER_URL).rstrip("/")
GITHUB_REPO = get_setting("GITHUB_REPO")
RESTART_EXCHANGE = get_setting("RESTART_EXCHANGE")
HOST_ACTIVE_TIMEOUT = int(os.environ.get("HOST_ACTIVE_TIMEOUT", "600"))
HOST_IMAGE = os.environ.get("HOST_IMAGE", "")
SSH_KEY = get_setting("SSH_KEY") if PROVIDER == "exe" else ""
SBX_WORKSPACE_ROOT = os.environ.get("SBX_WORKSPACE_ROOT", "")
PROVIDER_NEEDS_VALUE = PROVIDER == "docker-sbx"

# 70 seconds: a refresh and an outage in one run. An hour: one fetch, one more at a restart.
...
def call_issuer(path: str, body: dict | None = None) -> dict:
    request = urllib.request.Request(
...
def count_fetches(service: str) -> int:
    return call_issuer("/fetches")["services"].get(service, 0) - at_start.get(service, 0)
...
def run_on_host(command: str) -> str:
    done = subprocess.run(
...
def run_in_box(host: dict, script: str) -> str:
...
def make_entry(service: str) -> dict:
    return {
        "issuer": {
...
try:
    ...
    while host["status"] not in ("active", "error") and time.time() < deadline:
        time.sleep(5)
        host = {**host, **call_api("GET", f"/hosts/{host['id']}")[1]}
    check(
        "the host becomes active",
        host["status"] == "active",
        f"{name} {host['status']} {host.get('last_error') or ''}",
    )
    if PROVIDER_NEEDS_VALUE:
        time.sleep(8)
        check(
            "the issuer was asked for each secret at provisioning, and again at first sight",
            count_fetches("github") == 2 and count_fetches("anthropic") >= 2,
            f"github={count_fetches('github')} anthropic={count_fetches('anthropic')}",
        )
    else:
        check(
            "the issuer was not asked at boot",
            count_fetches("github") + count_fetches("anthropic") == 0,
        )

    env = run_in_box(
        host, 'echo "$ANTHROPIC_AUTH_TOKEN" | cut -c1-12; echo "$GH_TOKEN" | cut -c1-12'
    )
    check(
        "a plain session sees both placeholders", env.count("drk.") == 2, env.replace("\n", " | ")
    )
    tools = run_in_box(host, "command -v claude git gh | wc -l")
    check("claude, git, and gh are in the box", tools == "3", tools[-40:])

    if SBX_WORKSPACE_ROOT:
        listing = run_on_host(f"sbx secret ls --sandbox {name}")
        check(
        ...
        rows = [
            re.split(r"\s{2,}", line.strip()) for line in run_on_host("sbx secret ls").splitlines()
        ]
        check(
    ...
    answer = call_claude(host)
    check("claude -p answers ok", answer.endswith("ok"), answer[-60:])
    check(
        "gh api reaches the private repository",
        run_in_box(host, f"gh api repos/{GITHUB_REPO} --jq .full_name 2>&1 | tail -1")
        == GITHUB_REPO,
    )
    check(
        "gh pr list works",
        run_in_box(host, f"gh pr list -R {GITHUB_REPO} --limit 3 >/dev/null 2>&1 && echo listed")
        == "listed",
    )
    check(
        "the issuer was asked once for the github secret",
        count_fetches("github") == (2 if PROVIDER_NEEDS_VALUE else 1),
        f"github={count_fetches('github')}",
    )
    ...
    direct = get_status(
        host, f"--noproxy '*' {MODELS_URL} -H \"Authorization: Bearer $ANTHROPIC_AUTH_TOKEN\""
    )
    check("the placeholder sent straight to Anthropic is refused with 401", direct == "401", direct)
    wrong = get_status(
        host, f'{MODELS_URL} -H "Authorization: Bearer ${{ANTHROPIC_AUTH_TOKEN%??}}xx"'
    )
    check(
        "a wrong placeholder is refused"
        + ("" if PROVIDER_NEEDS_VALUE else " with 403 by the exchange"),
        wrong == ("401" if PROVIDER_NEEDS_VALUE else "403"),
        wrong,
    )

    # A fresh value first, then the issuer fails while that value nears its end.
    answer = call_claude(host)
    asked = count_fetches("anthropic")
    call_issuer("/fail", {"failing": True})
    try:
        time.sleep(20)
        answer = call_claude(host)
        time.sleep(2)
        check(
            "while the issuer fails, the last value keeps the box working",
            answer.endswith("ok") and count_fetches("anthropic") > asked,
            f"{answer[-40:]} failed attempts={count_fetches('anthropic') - asked}",
        )
    finally:
        call_issuer("/fail", {"failing": False})

    asked = count_fetches("github")
    ...
    check(
        ...
        count_fetches("github") - asked == 1,
        f"github={count_fetches('github') - asked}",
    )

    if PROVIDER == "docker" and shutil.which("docker"):
        # The entrypoint runs again at a restart: sshd comes back, the git setup stays single.
        restarted = subprocess.run(
    ...
    status, _ = call_api("DELETE", f"/hosts/{host['id']}")
    check("DELETE /hosts answers 204", status == 204, str(status))
    if SBX_WORKSPACE_ROOT:
        check(
            "teardown leaves no secret in sbx secret ls",
            "No secrets found" in run_on_host(f"sbx secret ls --sandbox {name}"),
        )
        check(
