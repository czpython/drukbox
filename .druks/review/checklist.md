...
2. truthiness, never `is (not) None` (spell it out only when `0`/`""`/`{}` are real
   distinct values; SQLAlchemy query expressions like `.is_(None)` are SQL, not
   control flow, and stay as they are)
3. no guards for values our own system produced — read them directly (guards on
   external data at a trust boundary — provider responses, caller input — are
   fine)
4. data models only at real boundaries: DB rows, wire schemas, provider result
   contracts; dicts/args between layers everywhere else
...
7. one noun per concept; never compose bags of loosely related fields
8. failures raise a typed error, never a sentinel return: no `value | error-string`
   union, no `X | None` as ok/fail, no `isinstance` on the error arm at the call
   site
9. wire boundary is minimal: single-field request body → `Body(..., embed=True)`,
...
10. positive conditions: `if value: do`, never `if not value:` bare-return followed
...
11. this repo is public: no internal ticket IDs or infra references in code,
    comments, commits, or PR bodies
...
The caller-facing surface (the HTTP API and its response shapes) is the product,
not the plumbing. Design it by writing the request/response example first: the
obvious call is the correct one, the correct one is short, and a newcomer gets it
right without reading the source or the docs. No exposed internals, no required
boilerplate, no knowledge of service mechanics leaking into the contract. If the
example needs a paragraph of setup or a caveat, the surface is wrong — redesign
it.
