...
@pytest.mark.asyncio
async def test_materialize_template_builds_and_returns_the_local_image_tag(tmp_path):
    api = _api_mock()
    docker = _docker_mock()
...
@pytest.mark.asyncio
async def test_delete_template_removes_the_local_image(tmp_path):
    api = _api_mock()
    docker = _docker_mock()
    provider = _provider(api, _settings(tmp_path), docker=docker)
...
@pytest.mark.asyncio
async def test_delete_template_translates_a_missing_image(tmp_path):
    api = _api_mock()
    docker = _docker_mock()
