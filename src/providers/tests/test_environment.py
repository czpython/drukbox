...
def test_export_quotes_each_value_for_the_shell():
    assert environment.get_export({"FOO": "bar", "QUOTE": "a b c", "EMPTY": ""}) == [
        "export FOO=bar",
        "export QUOTE='a b c'",
        "export EMPTY=''",
    ]
...
def test_export_rejects_invalid_env_names():
    with pytest.raises(ValueError, match="invalid VM environment variable name"):
        environment.get_export({"BAD-NAME": "v"})
...
def test_persist_accepts_values_pam_reads_back_unchanged(value):
    assert environment.get_persist({"KEY": value}) == [
...
def test_cloud_init_keeps_the_setup_script_shebang_first():
    out = environment.get_cloud_init(
        "#!/usr/bin/env bash\nset -e\necho ready\n", env={"FOO": "bar"}
    )
...
def test_cloud_init_without_env_is_the_setup_script():
    assert environment.get_cloud_init("#!/bin/sh\necho hi\n", env=None) == "#!/bin/sh\necho hi\n"


def test_trust_installs_the_proxy_ca_from_the_exported_variable_and_stops_on_failure():
    assert environment.get_install_ca({"SECRETS_PROXY_CA": "Q0VSVA==", "FOO": "bar"}) == [
...
def test_trust_uses_sudo_for_a_script_that_runs_as_a_user():
    assert environment.get_install_ca({"SECRETS_PROXY_CA": "Q0VSVA=="}, sudo=True) == [
...
def test_a_box_without_secrets_installs_nothing():
    assert environment.get_install_ca({"FOO": "bar"}) == []
    assert "ca-certificates" not in environment.get_cloud_init("echo hi", env={"FOO": "bar"})
...
def test_github_points_git_at_gh_and_sends_ssh_remotes_over_https():
    assert environment.get_github({"GH_TOKEN": "drk.a.github.b"}) == GIT_LINES


def test_github_uses_sudo_for_a_script_that_runs_as_a_user():
    assert environment.get_github({"GH_TOKEN": "drk.a.github.b"}, sudo=True) == [
...
def test_the_git_setup_runs_again_on_the_same_box(tmp_path):
    # git refuses to replace a key with several values.
    system = tmp_path / "gitconfig"
    script = "\n".join(environment.get_github({"GH_TOKEN": "drk.a.github.b"}))
    for _ in range(2):
        subprocess.run(
            ["sh", "-c", script],
...
def test_a_box_without_a_github_secret_gets_no_git_setup():
    assert environment.get_github({"ANTHROPIC_AUTH_TOKEN": "drk.a.anthropic.b"}) == []
    assert "git config" not in environment.get_cloud_init("echo hi", env={"FOO": "bar"})


def test_cloud_init_sets_git_up_after_the_ca_and_before_the_script():
    out = environment.get_cloud_init("echo hi", env={"GH_TOKEN": "drk.a.github.b"})
    assert out.splitlines()[-5:] == [*GIT_LINES, "echo hi"]


def test_cloud_init_installs_the_ca_after_the_env_and_before_the_script():
    out = environment.get_cloud_init("echo hi", env={"SECRETS_PROXY_CA": "Q0VSVA=="})
    assert out.splitlines() == [
        "#!/bin/sh",
