...
async def test_put_secret_writes_the_placeholder_and_the_edge_address_into_the_box():
    ...
    api.run_shell.return_value = "PATH=/usr/bin\nANTHROPIC_AUTH_TOKEN=old\n"
    ...
    assert name == "sb-one"
    assert script.startswith("printf") and script.endswith("> /etc/environment")
    assert "PATH=/usr/bin" in script
    assert "ANTHROPIC_AUTH_TOKEN=drk.abc.anthropic.x" in script
    assert "ANTHROPIC_AUTH_TOKEN=old" not in script
    assert "ANTHROPIC_BASE_URL=http://172.17.0.1:8080/api.anthropic.com" in script


@pytest.mark.asyncio
async def test_put_secret_keeps_the_base_path_the_client_expects():
    api = _api_mock()
    provider = DockerProvider(api, _settings(), secrets_exchange_url="http://172.17.0.1:8080")

    env = await provider.put_secret(vm="sb-one", service=_OPENAI, value="drk.abc.openai.x")

    assert env["OPENAI_BASE_URL"] == "http://172.17.0.1:8080/api.openai.com/v1"
...
async def test_list_and_delete_secrets_read_the_placeholders_back():
    ...
    assert f"ANTHROPIC_AUTH_TOKEN=drk.{host_hex}.anthropic.def" in script
    assert "OTHER=value" in script
