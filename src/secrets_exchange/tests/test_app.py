...
async def test_a_placeholder_is_exchanged_for_the_real_credential(edge) -> None:
    ...
    await _create_host(host_id, {"github": {"value": "ghs_real", "placeholder_sha256": stored}})

    response = await edge.get("/authorize", headers=_headers(placeholder, "/api.github.com/user"))
...
async def test_a_custom_service_gets_its_own_header_shape(edge) -> None:
    host_id = uuid.uuid4()
    placeholder, stored = mint(host_id, "acme")
    await _create_host(
        host_id,
        {
            "acme": {
                ...
                "endpoint_var": "",
                "value": "ak_live",
                "placeholder_sha256": stored,
            }
        },
    )

    response = await edge.get("/authorize", headers=_headers(placeholder, "/api.acme.test/v1"))
...
async def test_a_source_entry_waits_for_the_refresh_loop(edge) -> None:
    ...
    await _create_host(
        host_id,
        {"github": {"source": {"url": "https://mint.test/x"}, "placeholder_sha256": stored}},
    )
...
async def test_anything_else_is_refused_with_403_never_401(
    ...
    host_id = uuid.uuid4()
    real, stored = mint(host_id, "github")
    await _create_host(host_id, {"github": {"value": "ghs_real", "placeholder_sha256": stored}})
    presented = placeholder.format(
        host=host_id.hex, other=uuid.uuid4().hex, secret=real.rsplit(".", 1)[1]
    )
