...
- `hosts.service.HostService.put_secrets` mints a placeholder per secret when a host provisions and hands each secret to the provider's `secrets`.
- Each provider declares `secrets`, how a secret reaches its boxes: `ProxyInjection` on every provider but docker-sbx, `SbxInjection` on docker-sbx.
