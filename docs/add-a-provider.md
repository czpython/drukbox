...
placeholder and the proxy address in `env`, like every other variable, and the
provider has one job: put `env` where every session can read it. On Linux that
is `/etc/environment`, which PAM reads at login.
